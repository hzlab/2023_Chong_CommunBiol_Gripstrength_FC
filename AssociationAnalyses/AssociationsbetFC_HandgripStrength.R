# Perform linear regression to examine the association between functional connectivity measure and handgrip strength
...
#          FCfile: "SVA_IntraNetworkRegionalFCmeasures.csv",  outputfile = "Assoc_handgripstrength_SVAintraregionalFC.csv"
...
outputfile = '/XXX/AssociationAnalyses/Assoc_handgripstrength_segregation.csv'
...
data[, c('Age','Education','handgripstrength','TIV')] <- lapply(data[, c('Age','Education','handgripstrength','TIV')], as.numeric)

# Call function file
source(functiondir)

# Run linear regression model for each functional connectivity measure
for (i in 1:length(Varnames)){
  ...
  y=lm(FC~handgripstrength.centered + Age.centered + Gender + Education.centered + TIV.centered, data=data.copy)
  ...
  templm=data.frame(Dependent = 'handgripstrength',Predictor= Varnames[i], n = nrow(data.copy), templm, Rsquared = summary(y)$r.squared)
